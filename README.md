...
- **Agents speak karcarthy**: `run-authored` (an agent writes a flow) and
  ...
  never `eval`.
- **OpenTelemetry-ready**: wrap a harness with `karcarthy.otel/instrument` to
  emit spans for flow nodes, embedded functions, and agent calls.

```clojure
(require '[karcarthy.otel :as otel])

(k/run-flow (otel/instrument (k/mock-harness))
