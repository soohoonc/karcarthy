...
- A direct OpenAI Responses API transport and deterministic in-process fake
  model transport.
- A minimal coding profile with capability-derived instructions plus bounded
  `read`, `write`, `edit`, `bash`, and ripgrep-backed `search` tools.
...
- A paid coding-agent test that inspects and edits a temporary workspace.
