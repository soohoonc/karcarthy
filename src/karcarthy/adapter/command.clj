(ns karcarthy.adapter.command
  ...
  model, or a deterministic script just by swapping the adapter."
  (:require [clojure.string :as str]
            [karcarthy.core :as k]
            [karcarthy.proc :as proc]))

(defn command-adapter
  "Build a command adapter. `argv-or-fn` is either a fixed argv vector or a fn of
  the agent map returning an argv vector. Options:
    ...
    (command-adapter [\"cat\"])                         ; echoes stdin
    (command-adapter [\"tr\" \"a-z\" \"A-Z\"])               ; uppercases stdin
    (command-adapter (fn [a] [\"ollama\" \"run\" (:model a)]))"
