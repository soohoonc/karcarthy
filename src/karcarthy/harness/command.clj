...
(defn command-harness
  ...
  ([argv-or-fn {:keys [trim? env dir] :or {trim? true}}]
   (let [->argv (if (fn? argv-or-fn) argv-or-fn (constantly argv-or-fn))]
     (reify k/Harness
       (-run [_ agent prompt _opts]
         (let [argv    (->argv agent)
               sh-args (cond-> (concat argv [:in prompt])
                         env (concat [:env (merge (into {} (System/getenv)) env)])
                         dir (concat [:dir (str dir)]))
               {:keys [exit out err]} (apply shell/sh sh-args)]
           (k/result {:agent (:name agent)
                      :ok?   (zero? exit)
                      :text  (cond-> (or out "") trim? str/trim)
                      :error (when-not (zero? exit)
                               (str "command exited with status " exit))
                      :raw   {:exit exit :out out :err err :argv (vec argv)}})))))))
