...
## Why control outputs are EDN

The JSON bridge carries workflow shape, not host-language functions. Router and
critic agents still return EDN text, such as `{:route "launch"}` or
`{:accept? true}`, because the Clojure interpreter can parse and validate those
control messages the same way for Python, TypeScript, and Clojure callers.
