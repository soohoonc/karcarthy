...
plain object, but the agent UX should not be a set of one-line prompts. The
runnable file is `examples/typescript/tutorial_launch_brief.ts`.
...
This is the same JSON bridge as the Python tutorial. The important bit is that
your product can own the agent configuration UX while karcarthy keeps the
runtime boundary as portable workflow data.
