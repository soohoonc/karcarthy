---
title: TypeScript
description: Configure realistic agents in TypeScript and run them through karcarthy.
---

TypeScript sends workflow JSON to `bin/karcarthy`. The workflow is still a plain
object, but the agent configuration should not be a set of one-line prompts.
The runnable file is `examples/typescript/launch.ts`.
...
its own job, tool allowlist for the selected runner, boundaries, and output contract.
...
This is the same executable path as the Python guide. Your product can own the
